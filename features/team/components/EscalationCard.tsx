...
const TARGETS = ["Manager", "Front desk", "Everyone on shift"];

export function EscalationCard({ team }: { team: TeamState }) {
  const { escalation } = team;

  const summary = escalation.enabled
    ? `Right now: ${escalation.target} gets a second Telegram message ${escalation.minutes} minutes after a ticket arrives unread. It repeats once more at ${escalation.minutes * 2} min, then stops.`
    : "Escalation is off — only the routed role is notified, once.";
  ...
  return (
    <div className="rounded-tile border border-line bg-surface px-6 py-5.5">
      <div className="flex items-center gap-3">
        <div className="flex-1">
          ...
          <div className="mt-0.5 text-[12.5px] text-faint">
            Nobody should miss a new ticket at night.
          </div>
        </div>
        <Switch
      ...
      >
        ...
        <span>ticket is unread for</span>
        <NumberField
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) =>
              updateTeam({
                escalation: { ...escalation, target: event.target.value },
              })
            }
